...
std::vector<std::string> function_input_params__global_vector_string;
...
bool function_call_flag = false;
bool var_array_flag = false;
#ifdef __DEBUG__
...
void CminusBuilder::visit(syntax_fun_declaration &node) 
{
    ...
    //处理参数 使得有一个可用的参数列表 
    int param_num = 0;
    function_input_params__global_vector_llvmtype_p.clear();
    function_input_params__global_vector_string.clear();
    for(auto param: node.params)
    {
        ...
        {
            param -> accept(*this);
            function_input_params__global_vector_string.push_back(param->id);
            ++ param_num;
        }
    }
    llvm::Function* fun;
    ...
    {   
        if(node.params[i].get() -> isarray)
        {
            ...
            #ifdef __DEBUG__
            __DEBUG_PRINT(node.id)
            __DEBUG_PRINT("Array store created!")
            #endif
        }
        else
        {
...
void CminusBuilder::visit(syntax_param &node) 
{
    ...
    if (node.isarray)
    {
        if (node.type == TYPE_INT)
        {
            ...
            #ifdef __DEBUG__
            __DEBUG_PRINT(node.id)
            __DEBUG_PRINT("Array param found!")
            #endif
        }
    }
    else
...
void CminusBuilder::visit(syntax_var &node) 
{
    ...
    else
    {
        ...
        bool function_argument_flag = false;
        for (auto id : function_input_params__global_vector_string)
        {
            if(id == node.id)
            {
                function_argument_flag = true;
                break;
            }
        }
        std::vector<llvm::Value *> ind_list;
        if(!function_argument_flag)
        {
            auto zero = llvm::ConstantInt::get(llvm::Type::getInt32Ty(context),0,true);
            ind_list.push_back(zero);
        }
        ind_list.push_back(temp_array_index);
        auto array_ptr = function_argument_flag? builder.CreateGEP(builder.CreateLoad(temp_array), ind_list) :builder.CreateGEP(temp_array, ind_list);
